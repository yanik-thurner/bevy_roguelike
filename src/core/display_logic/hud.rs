...
pub fn update_health_bar(player_health_query: Query<&Health, With<Player>>, mut hp_bar_query: Query<&mut Style, With<HpBar>>, mut hp_text_query: Query<&mut Text, With<HpText>>) {
    if hp_bar_query.is_empty() || hp_text_query.is_empty() {
        return;
    }
...
pub fn setup_hud(mut commands: &mut Commands) {
