...
use num_traits::{Bounded, cast::FromPrimitive, PrimInt};
...
impl RandomUtil {
    pub fn derive_four_u32(seed: u32) -> (u32, u32, u32, u32) {
    ...
    pub fn pick_from_range<T: Bounded + PrimInt + FromPrimitive>(random: u32, range: impl RangeBounds<T>) -> T {
    ...
    pub fn rect_by_xy_and_dimensions<B: RangeBounds<i32>, C: RangeBounds<u32>>(random: u32, x_range: B, y_range: B, width_range: C, height_range: C) -> Rect {
        ...
        Rect::new(Position::new(x, y), Position::new(x + width as i32, y + height as i32))
    }

    pub fn pick_rect_inside_rect<C: RangeBounds<u32>>(random: u32, surrounding_rect: Rect, width_range: C, height_range: C) -> Result<Rect, String> {
    ...
    pub fn pick_position_in_rect(random: u32, surrounding_rect: Rect) -> Position {
        let x = Self::pick_from_range(random, surrounding_rect.x_range());
        let y = Self::pick_from_range(random, surrounding_rect.y_range());
        Position::new(x, y)
    }
}
...
impl Random
{
    ...
    pub fn rect_inside_rect<C: RangeBounds<u32>>(&mut self, surrounding_rect: Rect, width_range: C, height_range: C) -> Result<Rect, String> {
        RandomUtil::pick_rect_inside_rect(self.rng.next_u32(), surrounding_rect, width_range, height_range)
    }
