...
pub fn movement_system(mut commands: Commands, mut ev_wants_to_move: EventReader<WantsToMoveEvent>, map: Res<Map>, mut entities: Query<(Entity, &GridPosition, &mut FieldOfView), Or<(With<Enemy>, With<Player>)>>) {
    ...
    for ev in ev_wants_to_move.read() {
        let at_this_position = entities.iter().filter(|(entity, pos, _)| **pos == ev.destination && ev.entity != *entity && made_movements.values().all(|moved_from: &&GridPosition| **moved_from != ev.destination)).count();
