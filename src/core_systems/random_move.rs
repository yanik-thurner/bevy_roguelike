...
pub fn random_system(movers_query: Query<(Entity, &GridPosition), With<MovingRandomly>>, target_query: Query<(Entity, &GridPosition), With<Health>>, mut ev_wants_to_move: EventWriter<WantsToMoveEvent>, mut ev_wants_to_attack: EventWriter<WantsToAttack>) {
    for (current_mover_entity, current_mover_position) in movers_query.iter() {
        ...
        if let Some(enemy) = target {
            ev_wants_to_attack.send(WantsToAttack { attacker: current_mover_entity, victim: enemy });
        } else {
